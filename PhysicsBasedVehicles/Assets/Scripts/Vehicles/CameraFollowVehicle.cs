...
public class CameraFollowVehicle : MonoBehaviour
{
    ...
    [SerializeField] private bool needsActiveCommandModule = true;
    ...
    {
        if (Vehicle.GetActiveCommandModule() != null || !needsActiveCommandModule)
        {
            FollowTarget(Vehicle, Time.deltaTime);
        }
    }
    ...
    /// Follow the target.
    /// </summary>
    private void FollowTarget(Vehicle vehicle, float timeframe)
    {
        Transform target = vehicle.transform;
        Vector3 pos = target.position + target.TransformDirection(vehicle.Rb.centerOfMass);
        ...
        transform.position = pos + currentDirection.normalized * distance + target.up * height;
        transform.LookAt(pos + target.TransformDirection(lookOffset), Vector3.Cross(pos + lookOffset - transform.position, target.right));
